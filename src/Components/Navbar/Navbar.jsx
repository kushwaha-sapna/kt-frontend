    ...
    <nav ref={navRef} className="w-full bg-white fixed top-0 left-0 z-50 shadow-sm">
                ...
                >
                  ...
                  {openSubMenu === "legal" && (
                    <div className="absolute top-0 left-full ml-2 bg-white shadow-lg rounded-lg w-56 border">
                      ...
                      <Link to="/terms-and-condition" onClick={closeAllMenus} className="block px-4 py-2">Terms & Conditions</Link>
                    </div>
                  )}
                </div>
      ...
      <div className={`fixed top-0 left-0 h-full w-64 bg-white z-50 shadow-lg transform transition-transform duration-300 md:hidden ${
        ...
        <div className="p-5 space-y-4 mt-6">
          ...
          {/* COMPANY MOBILE */}
          <div onClick={() => handleMenuClick("company")} className="flex justify-between border-b pb-2 cursor-pointer">
            Company <span>{openMenu === "company" ? "−" : "+"}</span>
          </div>

          {openMenu === "company" && (
            <div className="ml-3 flex flex-col space-y-2 text-sm text-gray-600">

              <div onClick={() => handleSubMenuClick("aboutCompany")} className="flex justify-between cursor-pointer">
                About Company <span>{openSubMenu === "aboutCompany" ? "−" : "+"}</span>
              </div>

              {openSubMenu === "aboutCompany" && (
                <div className="ml-3 flex flex-col space-y-1">
                  <Link to="/about-us" onClick={closeAllMenus}>About Us</Link>
                  <Link to="/support" onClick={closeAllMenus}>Support</Link>
                  <Link to="/contact-us" onClick={closeAllMenus}>Contact</Link>
                </div>
              )}

              <div onClick={() => handleSubMenuClick("legal")} className="flex justify-between cursor-pointer">
                Legal & Policies <span>{openSubMenu === "legal" ? "−" : "+"}</span>
              </div>

              {openSubMenu === "legal" && (
                <div className="ml-3 flex flex-col space-y-1">
                  <Link to="/privacy-policy" onClick={closeAllMenus}>Privacy Policy</Link>
                  <Link to="/terms-and-condition" onClick={closeAllMenus}>Terms & Conditions</Link>
                </div>
              )}
            </div>
          )}
